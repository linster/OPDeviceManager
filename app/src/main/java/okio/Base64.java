...
final class Base64 {
    ...
    public static byte[] decode(String in) {
        ...
        while (pos < limit) {
            ...
            if (inCount % 4 != 0) {
                ...
                out[outCount] = (byte) ((byte) (word >> 16));
                outCount = outCount2 + 1;
                out[outCount2] = (byte) ((byte) (word >> 8));
                outCount2 = outCount + 1;
                out[outCount] = (byte) ((byte) word);
            }
            pos++;
            outCount = outCount2;
        }
        ...
        if (lastWordChars == 2) {
            outCount2 = outCount + 1;
            out[outCount] = (byte) ((byte) ((word << 12) >> 16));
    ...
    private static String encode(byte[] in, byte[] map) {
        ...
        for (int i = 0; i < end; i += 3) {
            int i2 = index + 1;
            out[index] = (byte) map[(in[i] & 255) >> 2];
            index = i2 + 1;
            out[i2] = (byte) map[((in[i] & 3) << 4) | ((in[i + 1] & 255) >> 4)];
            i2 = index + 1;
            out[index] = (byte) map[((in[i + 1] & 15) << 2) | ((in[i + 2] & 255) >> 6)];
            index = i2 + 1;
            out[i2] = (byte) map[in[i + 2] & 63];
        }
        switch (in.length % 3) {
            case 1:
                i2 = index + 1;
                out[index] = (byte) map[(in[end] & 255) >> 2];
                index = i2 + 1;
                out[i2] = (byte) map[(in[end] & 3) << 4];
                i2 = index + 1;
                out[index] = (byte) 61;
                index = i2 + 1;
                out[i2] = (byte) 61;
            ...
            case 2:
                ...
                out[index] = (byte) map[(in[end] & 255) >> 2];
                index = i2 + 1;
                out[i2] = (byte) map[((in[end] & 3) << 4) | ((in[end + 1] & 255) >> 4)];
                i2 = index + 1;
                out[index] = (byte) map[(in[end + 1] & 15) << 2];
                index = i2 + 1;
                out[i2] = (byte) 61;
                i2 = index;
                break;
