...
public final class C$Gson$Types {
    ...
    private static final class ParameterizedTypeImpl implements ParameterizedType, Serializable {
        ...
        public ParameterizedTypeImpl(Type ownerType, Type rawType, Type... typeArguments) {
            ...
            if (ownerType != null) {
                canonicalize = C$Gson$Types.canonicalize(ownerType);
            } else {
                canonicalize = null;
            }
    ...
    public static Type canonicalize(Type type) {
        if (type instanceof Class) {
            Class<?> c = (Class) type;
            return !c.isArray() ? c : new GenericArrayTypeImpl(C$Gson$Types.canonicalize(c.getComponentType()));
    ...
    public static boolean equals(Type a, Type b) {
        ...
        if (a instanceof ParameterizedType) {
            ...
            if (C$Gson$Types.equal(pa.getOwnerType(), pb.getOwnerType()) && pa.getRawType().equals(pb.getRawType())) {
    ...
    public static Type getArrayComponentType(Type array) {
        return !(array instanceof GenericArrayType) ? ((Class) array).getComponentType() : ((GenericArrayType) array).getGenericComponentType();
    }
    ...
    public static Type resolve(Type context, Class<?> contextRawType, Type toResolve) {
        ...
        if ((toResolve instanceof Class) && ((Class) toResolve).isArray()) {
            ...
            newComponentType = C$Gson$Types.resolve(context, contextRawType, componentType);
            if (componentType != newComponentType) {
                original2 = C$Gson$Types.arrayOf(newComponentType);
            }
            return original2;
            ...
            if (changed) {
                original3 = C$Gson$Types.newParameterizedTypeWithOwner(newOwnerType, original3.getRawType(), args);
            }
            return original3;
